...
```python
import pytest
from rampart.reporting import ReportSink, JsonFileReportSink

@pytest.fixture(scope="session")
def rampart_sinks() -> list[ReportSink]:
    return [
        JsonFileReportSink(output_dir=Path(".report")),
        MyDatabaseSink(connection="..."),
    ]
```
...
- [pytest Integration](../getting-started/pytest-integration.md) — Terminal summary, markers
- [CI Integration](ci-integration.md) — Using reports in CI
- [API: Reporting](../api/reporting.md) — Full API reference
